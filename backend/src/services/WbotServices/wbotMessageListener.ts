...
const handleOpenAi = async (
  ...
  // Pedido do Edison (2026-08-26): ele quer poder ajustar essas instruções
  // pelo painel (tela Open.Ai), sem precisar mexer em código toda vez. O
  // template inteiro (saudação, fluxo técnico, frases de Ação) agora vive
  // em Prompt.systemTemplate, editável - se estiver vazio, usa
  // DEFAULT_SYSTEM_TEMPLATE (ver helpers/DefaultSystemTemplate.ts, que
  // documenta os placeholders e seções condicionais disponíveis).
  const promptSystem = Mustache.render(
    prompt.systemTemplate?.trim() || DEFAULT_SYSTEM_TEMPLATE,
    {
      saudacao: saudacaoAtual,
      nome: sanitizeName(contact.name || "Amigo(a)"),
      maxTokens: prompt.maxTokens,
      cpfContexto,
      protocolo: buildTicketProtocol(ticket.id),
      primeiraMensagem: isFirstAiTurn,
      promptNegocio: prompt.prompt
    }
  );
